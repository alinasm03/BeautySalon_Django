...
class Test(TestCase):
    ...
    def test_panel_specialist(self):
        c = Client()
        c.login(username='admin', password='1234')
        sev1 = Service(name="test2", duration=1, price=1)
        sev1.save()
        service_db = Service.objects.filter(name='test2')
        response = c.post("/panel/specialist/", {"name": "Anetta", "range": "1",
                                                 "phone": "0500571106", "status": "1",
                                                 f"service_{sev1.id}": f"{sev1.id}"})

        self.assertEqual(response.status_code, 200)
        specialist = Master.objects.filter(name="Anetta")
        self.assertEqual(len(specialist), 1)

    def test_panel_services(self):
        c = Client()
        c.login(username='admin', password='1234')
        response = c.post("/panel/services/", {"name": "test_service", "duration": "1", "price": "10"})
        self.assertEquals(response.status_code, 200)
        all_services = Service.objects.filter(name="test_service")
        self.assertEquals(len(all_services), 1)

    def test_panel_schedule(self):
        c = Client()
        c.login(username='admin', password='1234')
        service = Service(name="test_calendar1", duration=1, price=1)
        service.save()
        specialist = Master(name="Alina_test", range=1, phone=997892233, status=True)
        specialist.save()
